  ...
  return (
    <div className="rf-container py-12 rf-fade-in" data-testid="account-page">
      <div className="mb-8 flex items-start justify-between gap-4 flex-wrap">
        <div>
          <div className="text-xs tracking-[0.2em] uppercase text-[#E11226] font-semibold">Mein Konto</div>
          <h1 className="font-display text-3xl md:text-4xl font-bold text-[#0A0A0A] mt-1">Hallo, {user?.name || user?.email}</h1>
        </div>
        {!profileComplete && (
      ...
      <Tabs defaultValue="bookings" className="w-full">
        <TabsList className="bg-[#F4F4F4] flex flex-wrap h-auto">
        ...
        <TabsContent value="bookings" className="mt-6">
          {loading ? (
            <div className="text-[#525252]">Lädt...</div>
          ) : bookings.length === 0 ? (
            <div className="text-center py-16 border border-dashed border-[#D4D4D4] rounded-lg">
              <Car className="w-10 h-10 text-[#A3A3A3] mx-auto mb-3" />
              <div className="text-[#525252] mb-4">Du hast noch keine Buchungen.</div>
              <Link to="/katalog"><Button className="bg-[#E11226] hover:bg-[#C20E1F]">Jetzt Fahrzeug suchen</Button></Link>
            </div>
          ) : (
            <div className="space-y-4">
              {bookings.map((b) => {
                ...
                return (
                  <div key={b.id} className="bg-white border border-[#E5E5E5] rounded-lg p-5 flex flex-col md:flex-row gap-4" data-testid={`booking-row-${b.id}`}>
                    <img src={b.vehicle_image} alt="" className="w-full md:w-48 h-36 md:h-28 object-cover rounded-md border border-[#E5E5E5]" />
                    <div className="flex-1">
                      <div className="flex items-start justify-between gap-3 flex-wrap">
                        <div>
                          <div className="text-xs text-[#525252]">{b.vehicle_brand}</div>
                          <div className="font-display font-semibold text-lg text-[#0A0A0A]">{b.vehicle_name}</div>
                        </div>
                      ...
                      <div className="mt-3 flex justify-end">
                        <a
                          ...
                          className="inline-flex items-center gap-1.5 text-xs font-semibold text-[#E11226] hover:underline"
        ...
        <TabsContent value="profile" className="mt-6">
          <div className="max-w-2xl bg-white border border-[#E5E5E5] rounded-lg p-6">
            <h2 className="font-display font-semibold text-lg text-[#0A0A0A] mb-5">Persönliche Daten</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <Field label="E-Mail"><Input value={user?.email} disabled className="bg-[#F4F4F4]" /></Field>
            ...
            <Button onClick={saveProfile} disabled={saving} className="mt-6 bg-[#E11226] hover:bg-[#C20E1F]" data-testid="profile-save">
        ...
        <TabsContent value="address" className="mt-6">
          <div className="max-w-2xl bg-white border border-[#E5E5E5] rounded-lg p-6">
            <h2 className="font-display font-semibold text-lg text-[#0A0A0A] mb-5">Rechnungs- & Wohnadresse</h2>
            ...
            <Button onClick={saveProfile} disabled={saving} className="mt-6 bg-[#E11226] hover:bg-[#C20E1F]" data-testid="addr-save">
        ...
        <TabsContent value="business" className="mt-6">
          <div className="max-w-2xl bg-white border border-[#E5E5E5] rounded-lg p-6">
            <div className="flex items-start justify-between mb-5 gap-4">
              <div>
                <h2 className="font-display font-semibold text-lg text-[#0A0A0A]">Geschäftskunde</h2>
                <p className="text-sm text-[#525252] mt-1">Aktiviere diese Option, wenn du im Namen eines Unternehmens buchst.</p>
              </div>
              <Switch
            ...
            {form.is_business && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5 pt-5 border-t border-[#E5E5E5]">
            ...
            <Button onClick={saveProfile} disabled={saving} className="mt-6 bg-[#E11226] hover:bg-[#C20E1F]" data-testid="biz-save">
              <Save size={14} className="mr-2" /> {saving ? "Speichert..." : "Speichern"}
            </Button>
          </div>
        </TabsContent>
...
function Info({ icon: Icon, label, value }) {
  return (
    <div>
      <div className="text-[#525252] flex items-center gap-1"><Icon size={11} /> {label}</div>
      <div className="font-semibold text-[#0A0A0A] truncate">{value}</div>
    </div>
  );
}
